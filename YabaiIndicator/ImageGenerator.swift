...
private func drawText(symbol: NSString, color: NSColor, size: CGSize) {
    let attrs: [NSAttributedString.Key: Any] = [
        .font: NSFont.systemFont(ofSize: Constants.fontSize), .foregroundColor: color,
    ]
...
func generateImage(symbol: NSString, active: Bool, visible: Bool) -> NSImage {
    ...
    return ImageCache.image(for: cacheKey) {
        ...
        if active || visible {
            ...
            strokeColor.setFill()
            NSBezierPath(
                roundedRect: canvas, xRadius: Constants.cornerRadius,
            ...
            image.lockFocus()
            imageFill.draw(
                in: canvas, from: NSZeroRect, operation: .sourceOut, fraction: active ? 1.0 : 0.8)
            imageStroke.draw(
                in: canvas, from: NSZeroRect, operation: .destinationOut,
                fraction: active ? 1.0 : 0.8)
            image.unlockFocus()
        } else {
            image.lockFocus()
            strokeColor.setStroke()
            let path = NSBezierPath(
                roundedRect: canvas.insetBy(dx: 0.5, dy: 0.5), xRadius: Constants.cornerRadius,
                yRadius: Constants.cornerRadius)
            path.stroke()
            drawText(symbol: symbol, color: strokeColor, size: size)
            image.unlockFocus()
...
func drawWindows(in content: NSRect, windows: [Window], display: Display) {
    ...
    guard contentSize.width > 0, contentSize.height > 0,
        ...
        logWarning("Invalid size for drawWindows: content=\(contentSize), display=\(displaySize)")
        return
    }

    let scaling =
        displaySize.height > displaySize.width
        ? displaySize.height / contentSize.height : displaySize.width / contentSize.width

    // Guard against division by zero and invalid values
    guard scaling > 0 && scaling.isFinite else {
        logWarning(
            "Invalid scaling value (\(scaling)) for display. Display size: \(displaySize), Content size: \(contentSize)"
        )
        return
    }
    ...
    let xoffset =
        (displaySize.height > displaySize.width
            ? (contentSize.width - displaySize.width / scaling) / 2 : 0) + contentOrigin.x
    let yoffset =
        (displaySize.height > displaySize.width
            ? 0 : (contentSize.height - displaySize.height / scaling) / 2) + contentOrigin.y
...
func generateImage(active: Bool, visible: Bool, windows: [Window], display: Display) -> NSImage {
    ...
    let windowPositions = windows.map {
        "\($0.frame.origin.x),\($0.frame.origin.y),\($0.frame.size.width),\($0.frame.size.height)"
    }.joined(separator: "|")
    let cacheKey =
        "windows_\(active)_\(visible)_\(display.id)_\(windows.count)_\(windowPositions.hashValue)"
